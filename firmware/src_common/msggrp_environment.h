...
// Possible MessageTypes: Get, Status, AckStatus
// Validity: test
// Length w/o Header + HeaderExtension: 216 bits
...
// Description: Typical Particle Size [1/100 μm]. Use 65535 when value invalid.

// Set TypicalParticleSize (UIntValue)
// Offset: (uint16_t)__HEADEROFFSETBITS + 0, length bits 16, min val 0, max val 65535
static inline void msg_environment_particulatematter_set_typicalparticlesize(uint32_t val)
{
  array_write_UIntValue((uint16_t)__HEADEROFFSETBITS + 0, 16, val, bufx);
}
...
{
  return array_read_UIntValue32((uint16_t)__HEADEROFFSETBITS + 0, 16, 0, 65535, bufx);
}

// Size (UIntValue[5])
// This sub-element with 8 bits is part of an element with 40 bits in a structured array.
...
// Set Size (UIntValue)
// Offset: (uint16_t)__HEADEROFFSETBITS + 16 + (uint16_t)index * 40, length bits 8, min val 0, max val 255
static inline void msg_environment_particulatematter_set_size(uint8_t index, uint32_t val)
{
  array_write_UIntValue((uint16_t)__HEADEROFFSETBITS + 16 + (uint16_t)index * 40, 8, val, bufx);
}
...
{
  return array_read_UIntValue32((uint16_t)__HEADEROFFSETBITS + 16 + (uint16_t)index * 40, 8, 0, 255, bufx);
}

// MassConcentration (UIntValue[5])
// This sub-element with 16 bits is part of an element with 40 bits in a structured array.
// Description: Mass concentration [1/10 μg/m3], considering the defined size. Use 65535 when the value is invalid.

// Set MassConcentration (UIntValue)
// Offset: (uint16_t)__HEADEROFFSETBITS + 24 + (uint16_t)index * 40, length bits 16, min val 0, max val 65535
static inline void msg_environment_particulatematter_set_massconcentration(uint8_t index, uint32_t val)
{
  array_write_UIntValue((uint16_t)__HEADEROFFSETBITS + 24 + (uint16_t)index * 40, 16, val, bufx);
}
...
{
  return array_read_UIntValue32((uint16_t)__HEADEROFFSETBITS + 24 + (uint16_t)index * 40, 16, 0, 65535, bufx);
}

// NumberConcentration (UIntValue[5])
// This sub-element with 16 bits is part of an element with 40 bits in a structured array.
// Description: Number concentration [1/10 #/cm3], considering the defined size. Use 65535 when the value is invalid.

// Set NumberConcentration (UIntValue)
// Offset: (uint16_t)__HEADEROFFSETBITS + 40 + (uint16_t)index * 40, length bits 16, min val 0, max val 65535
static inline void msg_environment_particulatematter_set_numberconcentration(uint8_t index, uint32_t val)
{
  array_write_UIntValue((uint16_t)__HEADEROFFSETBITS + 40 + (uint16_t)index * 40, 16, val, bufx);
}
...
{
  return array_read_UIntValue32((uint16_t)__HEADEROFFSETBITS + 40 + (uint16_t)index * 40, 16, 0, 65535, bufx);
}
