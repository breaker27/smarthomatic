...
public class SourceCodeGeneratorPacket
{
	...
	private int generateDataFieldDefs(Node dataNode, boolean useHeaderOffset, int offset, String functionPrefix, StringBuilder sb, ArrayList<String> dataFields, int arrayLength, int structLengthBits) throws TransformerException
	{
		...
		for (int e = 0; e < childs.getLength(); e++)
		{
			...
			else if (element.getNodeName().equals("FloatValue"))
			{
				String ID = Util.getChildNodeValue(element, "ID");
				dataFields.add(ID);

				int bits = 32; // Float has fixed size
				if (!isArray)
					structLengthBits = bits;

				sb.append("// " + ID + " (FloatValue" + arrayNameSuffix + ")" + newline);
				...
				sb.append("// Set " + ID + " (FloatValue)" + newline);
				sb.append("// Offset: " + offsetStr + ", length bits " + bits + newline);

				sb.append("static inline void " + functionPrefix + "_set_" + ID.toLowerCase() + "(" + funcParam + "float val)" + newline);
				sb.append("{" + newline);
				sb.append("  array_write_FloatValue(" + offsetStr + ", val, bufx);" + newline);
				sb.append("}" + newline);
				sb.append(newline);

				// GET

				sb.append("// Get " + ID + " (FloatValue)" + newline);
				sb.append("// Offset: " + offsetStr + ", length bits " + bits + newline);

				sb.append("static inline float " + functionPrefix + "_get_" + ID.toLowerCase() + "(" + funcParam2 + ")" + newline);
				sb.append("{" + newline);
				sb.append("  return array_read_FloatValue(" + offsetStr + ", bufx);" + newline);
				sb.append("}" + newline);
				sb.append(newline);
	...
	private int getOvrArrayStructLen(Node dataNode)
	{
		...
		for (int e = 0; e < childs.getLength(); e++)
		{
			...
			else if (nodeName.equals("FloatValue"))
			{
				len += 32;
			}
	...
	public static String genCopyrightNotice()
	{
		return "/*" + newline +
				...
				"* Copyright (c) 2013..2019 Uwe Freese" + newline +
				"*" + newline +
